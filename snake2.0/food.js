function s2SpawnBigFood() {
    ...
    s2BigFoods.push({ x, y, color: `hsl(${Math.floor(Math.random()*360)},90%,60%)` });
}
...
function s2SpawnFood() {
    ...
    s2Foods.push({ x, y, color: `hsl(${Math.floor(Math.random()*360)},90%,60%)` });
}
...
function s2DropFoodTrail(snakeArr) {
    ...
    for (let i = 0; i < snakeArr.length; i += 4) {
        ...
        if (isBig) {
            ...
            if (s2Foods.length + s2BigFoods.length < S2_MAX_FOOD) s2BigFoods.push({ x: snakeArr[i].x, y: snakeArr[i].y, color: `hsl(${Math.floor(Math.random()*360)},90%,60%)` });
        } else {
            if (s2Foods.length + s2BigFoods.length < S2_MAX_FOOD) s2Foods.push({ x: snakeArr[i].x, y: snakeArr[i].y, color: `hsl(${Math.floor(Math.random()*360)},90%,60%)` });
        }
    }
}